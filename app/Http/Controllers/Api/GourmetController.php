...
class GourmetController extends Controller {
    ...
    public function wxPay($sNo = '') {
        ...
        return WeChatPay::getInstance()->copy()
            ->resetGateway('WechatPay_App', 'http://andou.zhuosongkj.com/api/common/gourmet')
            ->createOrder(
                $sNo,
                $pay_money,
                '安抖本地生活-消费',
...
//            var_dump($foods);exit();
    ...
    public function refund() {
        ...
        $ress = DB::table('orders')->where('order_sn', $all['order_sn'])->update(['status' => 60]);
        if ($res && $ress) {
            DB::commit();
